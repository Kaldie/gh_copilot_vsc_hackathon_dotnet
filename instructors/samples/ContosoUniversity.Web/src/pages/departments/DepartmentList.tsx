  ...
  const columns: Column<DepartmentListDto>[] = [
    {
      ...
      header: "Name",
      render: (d) => (
        <Link to={`/departments/${d.departmentId}`} className="font-medium text-gray-900 dark:text-gray-100 hover:text-gray-600 dark:hover:text-gray-400">
          {d.name}
        </Link>
      ),
    },
    ...
    { key: "administrator", header: "Administrator", render: (d) => d.administratorName ?? "—" },
    {
      ...
      header: "",
      render: (d) => (
        <Link to={`/departments/${d.departmentId}`} className="text-sm font-medium text-gray-500 dark:text-gray-400 transition-colors hover:text-gray-900 dark:hover:text-gray-100">View →</Link>
      ),
    },
  ];
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Departments</h1>
        <Link
