  ...
  const columns: Column<StudentListDto>[] = [
    {
      ...
      header: "Last Name",
      sortable: true,
      render: (s) => <Link to={`/students/${s.id}`} className="font-medium text-gray-900 dark:text-gray-100 hover:text-gray-600 dark:hover:text-gray-400">{s.lastName}</Link>,
    },
    {
      key: "FirstName",
      header: "First Name",
    ...
    },
    {
      ...
      header: "",
      render: (s) => (
        <Link to={`/students/${s.id}`} className="text-sm font-medium text-gray-500 dark:text-gray-400 transition-colors hover:text-gray-900 dark:hover:text-gray-100">View →</Link>
      ),
    },
  ];
