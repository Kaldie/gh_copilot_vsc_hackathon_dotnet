    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link to={`/students/${student.id}/edit`} className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50">Edit</Link>
          <Link to={`/students/${student.id}/delete`} className="rounded-lg border border-red-200 bg-white px-4 py-2 text-sm font-medium text-red-600 transition-colors hover:bg-red-50">Delete</Link>
        </div>
      </div>
      ...
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-sm font-medium text-gray-400">Enrollments</h2>
        <Link to="/students" className="text-sm text-gray-400 transition-colors hover:text-gray-700">← Back to List</Link>
      </div>

      {student.enrollments.length === 0 ? (
        <p className="mb-4 text-sm text-gray-400">No enrollments yet.</p>
      ) : (
          ...
          <table className="min-w-full">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="px-5 py-3 text-left text-xs font-medium text-gray-400">Grade</th>
                <th className="px-5 py-3 text-left text-xs font-medium text-gray-400"></th>
              </tr>
            </thead>
            <tbody>
              {student.enrollments.map((e, idx) => (
                <tr key={e.enrollmentId} className={idx < student.enrollments.length - 1 ? "border-b border-gray-50" : ""}>
                  <td className="whitespace-nowrap px-5 py-3.5 text-sm text-gray-700">{e.courseTitle}</td>
                  <td className="whitespace-nowrap px-5 py-3.5 text-sm">
                    <select
                      ...
                      className="rounded-lg border border-gray-200 bg-gray-50 px-2 py-1 text-sm text-gray-700 transition-colors focus:border-gray-300 focus:bg-white focus:outline-none"
                    >
