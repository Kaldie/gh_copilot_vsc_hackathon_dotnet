  ...
  const columns: Column<CourseListDto>[] = [
    {
      ...
      header: "Course ID",
      render: (c) => (
        <Link to={`/courses/${c.courseId}`} className="font-medium text-gray-900 hover:text-gray-600">
          {c.courseId}
        </Link>
      ),
    },
    ...
    { key: "department", header: "Department", render: (c) => c.departmentName },
    {
      ...
      header: "",
      render: (c) => (
        <Link to={`/courses/${c.courseId}`} className="text-sm font-medium text-gray-500 transition-colors hover:text-gray-900">View →</Link>
      ),
    },
  ];
