  ...
  const instructorColumns: Column<InstructorListDto>[] = [
    {
      ...
      render: (i) => (
        <button
          ...
          className={`text-left font-medium transition-colors ${selectedInstructor === i.id ? "text-gray-900 underline" : "text-gray-900 hover:text-gray-600"}`}
        >
          {i.lastName}
        </button>
      ),
    ...
    { key: "office", header: "Office", render: (i) => i.officeLocation ?? "" },
    {
      ...
      header: "",
      render: (i) => (
        <Link to={`/instructors/${i.id}`} className="text-sm font-medium text-gray-500 transition-colors hover:text-gray-900">View →</Link>
      ),
    },
  ];
  ...
  const courseColumns: Column<CourseListDto>[] = [
    {
      ...
      render: (c) => (
        <button
          ...
          className={`text-left font-medium transition-colors ${selectedCourse === c.courseId ? "text-gray-900 underline" : "text-gray-900 hover:text-gray-600"}`}
        >
          {c.courseId}
        </button>
      ),
  ...
  return (
    <div>
      ...
      {selectedInstructor !== null && courses.length > 0 && (
        <div className="mt-8">
          <h2 className="mb-3 text-sm font-medium text-gray-400">Courses Taught</h2>
          <DataTable
      ...
      {selectedCourse !== null && (
        <div className="mt-8">
          <h2 className="mb-3 text-sm font-medium text-gray-400">Enrolled Students</h2>
          <DataTable
