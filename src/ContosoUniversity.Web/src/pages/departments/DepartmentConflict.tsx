...
export default function DepartmentConflict() {
  ...
  if (!state) {
    return (
      <div className="text-center">
        <p className="mb-4 text-gray-500">No conflict data available.</p>
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="mb-2 text-xl font-semibold text-gray-900">Concurrency Conflict</h1>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Database Value</th>
              <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Your Value</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200">
            {fields.map((f) => (
              <tr key={f.label} className={f.current !== f.submitted ? "bg-yellow-50" : ""}>
                <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">{f.label}</td>
                <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-700">{f.current}</td>
                <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-700">{f.submitted}</td>
              </tr>
            ))}
          </tbody>
        </table>
